> **2026-09-08 品牌更名注记**：本项目品牌更名为 **AgentBazaar（智能体集市）**。本文档历史记录中的 "AgentMarket" 均指同一项目（仓库技术名 agentmarket 不变，协议/事件/任务格式不受影响）。
...
- **D-123**：稳定币兑换再评估（2026-09-09）——不引（维持 D-120，论证升级：需求错配+合规不可承受）；吸引参与的正确杠杆=任务供给/声誉可迁移/INTEROP 凭证互认/零摩擦参与；稳定币仅作为运营者层商业化插件 backlog（需合规主体），非协议层功能。
